<template>
  <div class="pages">
    <header class="title-2" id="content">
      <span>FUTBOL BÜLTEN LİG DÜZENLE</span>
    ...
    <ul class="main-set-list">
      ...
      <li v-for="(list, index) in leagues" :key="index" class="fix-li">
        ...
        <input v-model="list[$store.state.auth.user]" type="checkbox" class="check-inp" @change="updateLeagues(list[$store.state.auth.user], list.lid)">
...
export default {
  ...
  methods: {
    ...
    async getLeagues () {
      this.loader = true
      await axios.get('/api/home/account/leagues/soccer').then((result) => {
    ...
    async updateLeagues (state,lid) {
      this.setloader = lid
      await axios.post('/api/home/account/soccerupdate', {type:'norma365',state:state, lid:lid}).then((result) => {
        this.done = lid
        this.setloader = ''
        setTimeout(function () {
          this.done = ''
