<template>
  <div class="pages">
    <header class="title-2" id="content">
      ...
      <select v-if="checkA2 || checkG1" v-model="userBox" class="selectbox box-account" @change="getDetailsInfo()">
    ...
    async getDetailsInfo() {
      try {
        ...
        Promise.all([activeinfo, woninfo, lostinfo, totalinfo, credit, subcredit]).then(result => {
          if (result[0].data.activeinfo) {
            this.activeInfo = result[0].data.activeinfo
          }else{this.activeInfo = {}}
          if (result[1].data.woninfo) {
            this.wonInfo = result[1].data.woninfo
          }else{this.wonInfo = {}}
          if (result[1].data.lostinfo) {
            this.lostInfo = result[2].data.lostinfo
          }else{this.lostInfo = {}}
          if (result[3].data.totalinfo) {
