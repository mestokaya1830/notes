  ...
  <div v-if="checkRole" class="pages">
    ...
    <ul>
      <li class="title-ligth">
        ...
        <span v-if="checkAdmin" class="uitems bitems-title">LİMİT</span>
        <span v-if="checkAdmin || checkSubadmin" class="uitems bitems-title">BAKİYE</span>
        <span v-if="checkBoss || checkSuperadmin" class="uitems bitems-title">ÜYE SAYISI</span>
    ...
    <ul class="bet-summary-con-admin">
      <li v-for="list in users" :key="list._user">
        <nuxt-link class="list" :to="'/home/account/users/' + list.user">
          ...
          <span v-if="checkAdmin" class="uitems-list">{{ list.credit | currency('', 0) }}</span>
          <span v-if="checkAdmin || checkSubadmin" class="uitems-list">
            {{ list.creditremain | currency('', 0) }}
            <div v-if="list.role === 'Subadmin'">
              (<span class="cl-r subcredit">{{list.subcredit | currency('', 0)}}</span>)
            </div>
          </span>
          <span v-if="checkBoss || checkSuperadmin" class="uitems-list">{{ list.userlimit }} / {{ list.userlimit - list.usedlimit }}</span>
...
export default {
  ...
  computed: {
    checkSubadmin() {
      if (
        this.$store.state.auth.role === "Subadmin"
      ) {
        return true;
      }
    },
    checkMember() {
      if (
        this.$store.state.auth.role === "Member"
      ) {
        return true;
      }
    },
  ...
  methods: {
    async getUsers() {
      ...
      await axios.get("/api/home/account/users").then(result1 => {
        this.isLoader = false
        const users = result1.data.data.map(item => {if (item.role === 'Subadmin') {return item.user}}).filter(item => item !== undefined)
