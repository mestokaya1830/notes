...
function basket() {
  ...
  const b = JSON.parse(result.getBody('utf8'))
  const pagenumber = Math.ceil(b.pager.total/b.pager.per_page)
    const urls = []
    ...
    fetchData().then(arrayOfResponses => {
      ...
      if (newarraybasket.length > 0) {
        ...
        const final = storearray.filter(item => {return moment(item.Date).format('YYYY-MM-DD HH:mm') > start && moment(item.Date).format('YYYY-MM-DD HH:mm') < end})
        if (final.length > 0) {
          let fixtures = []
          let home = ''
          let away = ''
...
router.get("/soccerfixtureodds/:id", cacheMiddleware(1800),  (req, res)=>{
  try {
    const result = request('GET', `https://api.betsapi.com/v1/bwin/event?token=23479-hVapIXdy0gkZIo&event_id=${req.params.id}`,{json: {results: 'results'}})
...
router.get("/basketinplay", cacheMiddleware(40),  (req, res)=>{
  try {
    const result = request('GET', `https://api.betsapi.com/v1/bwin/inplay?token=23479-hVapIXdy0gkZIo&sport_id[]=7`,{json: {results: 'results'}})
    const filterstore = JSON.parse(result.getBody('utf8')).results.filter(item => item.Scoreboard.started == true && item.Scoreboard.period !== 'Reg. Time Over')
    res.json(filterstore)
  } catch (error) {console.log(error)}
})
