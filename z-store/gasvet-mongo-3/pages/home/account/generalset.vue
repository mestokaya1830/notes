  ...
  <div v-if="$store.state.auth.role === 'Admin'" class="pages po-r">
    ...
    <ul>
      ...
      <div class="main-set-list">
        ...
        <li class="fix-li user-id-section">
          <div class="fix-1">
            <input type="checkbox" class="set-cb" :id="users.multilogin" v-model="users.multilogin" @change="updateMultiLogin()" />
            <span class="list-mn">Coklu Giriş</span>
...
export default {
  ...
  methods: {
    ...
    async updateMultiLogin() {
      await axios.post("/api/multilogin", { multilogin: this.users.multilogin }).then(result => {
