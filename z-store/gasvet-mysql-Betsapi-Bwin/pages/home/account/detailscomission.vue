      ...
      <div class="bet-con">
        <ul>
          <li class="title-ligth">
              ...
              <span class="bitems cp">
                <span>
                  <select v-model="userBox" class="selectbox box-bet select-owner" @change="getBetSummary()">
                    ...
                    <option value="Üyeler" selected>Hepsi</option>
            ...
            <span class="bitems bstate" v-if="!checkB1 || !checkS1">KOMİSYON</span>
          </li>
        </ul>
        <ul class="bet-summary-con po-r">
          <div class="list-bet"  v-for="(list,index) in betSummary" :key="index" :class="list.state">
            ...
            <span class="bitems-list bstate">{{(list.baseearn - list.earn) | currency('', 2)}}</span>
          </div>
...
export default {
  ...
  computed: {
    checkRole () {
      if (this.$store.state.auth.role === 'G1' || this.$store.state.auth.role === 'U1') {
        return true
      }
    },
    getComission(){
  ...
  methods: {
    ...
    getTotal(value){
      ...
      this.calcComission = won.reduce((acc, item)=>{
        return acc + (item.baseearn - item.earn)
      },0)
    },
