<template>
  <div class="pages">
     <header class="title-2">
       <span class="set-title">GEÇMİŞ RAPOR</span>
          ...
          <div class="bet-con">
            <ul>
              ...
              <li class="set-li set-li-ins">
                ...
                <span class="set-li-item">
                  <span class="stlt">Kaybeden Girdi : </span>
                  <span>{{lost.amount | currency('', 2)}}</span>
                </span>
              </li>
              <li class="set-li set-li-ins hr-result">
                <span class="set-li-item">
...
export default {
  ...
  methods: {
    ...
    async getBetInfo () {
      this.isLoader = true
      await axios.post('/api/home/account/historyreport/',{userbox:this.userBox}).then((result) => {
        if (result.data.totalinfo) {
