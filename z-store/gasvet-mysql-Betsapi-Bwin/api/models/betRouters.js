  ...
  try {
    ...
    //halftime----------------
      if (halftimescors.length > 0) {
        const conn = await mysql.getConnection()
        conn.release()
        let halftimebets = await conn.query('SELECT id, betid, user, scoreid, matches, games, options FROM bets WHERE state = ? AND games = ?',['Aktif', 'Maç Bahsi'])
        if (halftimebets.length > 0) {
          let homeresult = halftimebets[0].filter(item1 => {return halftimescors.some(item2 => (item2.matches.trim() === item1.matches.trim()))})
          halftimescors.forEach(item2 => {
            homeresult.forEach(item => {
...
router.put('/cancelbet', wrapAsync(async (req, res) => {
  const conn = await mysql.getConnection()
  conn.release()
  await conn.query('UPDATE betsummary SET state = ? WHERE id = ?', ['iptal', req.body.id])
  await conn.query('UPDATE bets SET state = ? WHERE betid = ?', ['iptal', req.body.id])
  await conn.query('UPDATE users SET creditremain = creditremain + ? WHERE user = ?', [req.body.amount, req.body.user])
  const betsummary = await conn.query('SELECT admin, user, amount FROM betsummary WHERE id = ?', [req.body.id])
  const users = await conn.query('SELECT creditremain FROM users WHERE user = ?', [betsummary[0][0].user])
  let gamelog = {
    betsumid: req.body.id,
    user: betsummary[0][0].user,
    admin: betsummary[0][0].admin,
    date: new Date(),
    amount: betsummary[0][0].amount,
    creditremain: users[0][0].creditremain,
    ope: 'Eklendi',
    des: 'iptal'
  }
  await conn.query('INSERT INTO gamelog SET ?', [gamelog])
  await conn.query('UPDATE setlimit SET maxkisremain = maxkisremain + 1 WHERE user = ?', [req.body.user])
  res.json({ code: 200, message: 'Kupon iptal edildi' })
}))
router.put('/returnbet', wrapAsync(async (req, res) => {
  const conn = await mysql.getConnection()
  conn.release()
  await conn.query('UPDATE betsummary SET state = ? WHERE id = ?', ['Aktif', req.body.id])
  await conn.query('UPDATE bets SET state = ? WHERE betid = ?', ['Aktif', req.body.id])
  await conn.query('UPDATE users SET creditremain = creditremain - ? WHERE user = ?', [req.body.amount, req.body.user])
  const betsummary = await conn.query('SELECT admin, user, amount FROM betsummary WHERE id = ?', [req.body.id])
  const users = await conn.query('SELECT creditremain FROM users WHERE user = ?', [betsummary[0][0].user])
  let gamelog = {
    ...
    user: betsummary[0][0].user,
    admin: betsummary[0][0].admin,
    superadmin: betsummary[0][0].admin,
    date: new Date(),
    amount: betsummary[0][0].amount,
    creditremain: users[0][0].creditremain,
    ope: 'Eklendi',
...
router.post('/betsummary/quickdatefilter', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    if (req.body.statebox === 'Hepsi') {
      const datefilter = await conn.query('SELECT * FROM betsummary WHERE user = ? AND state != "iptal" AND date LIKE "' + req.body.quickdate + '%" ORDER BY id DESC', [req.session.auth.user, req.body.datestart, req.body.datefinish])
      res.json({ code: 200, datefilter: datefilter[0] })
    } else {
      const datefilter = await conn.query('SELECT * FROM betsummary WHERE user = ? AND state = ? AND date LIKE "' + req.body.quickdate + '%" ORDER BY id DESC', [req.session.auth.user, req.body.statebox, req.body.datestart, req.body.datefinish])
      res.json({ code: 200, datefilter: datefilter[0] })
    }
  } else if (req.session.auth.role === 'G1'){
    if (req.body.userbox === 'Üyeler') {
...
router.post('/betsummarynextgoals', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    ...
    const betsummary = await conn.query(`SELECT * FROM betsummary WHERE user = ? AND id IN(${getid}) ORDER BY id DESC`, [req.session.auth.user])
    res.json({ betsummary: betsummary[0] })
  } else if (req.session.auth.role === 'G1') {
    ...
    const getid = betsid[0].map(item => item.betid)
    const betsummary = await conn.query(`SELECT * FROM betsummary WHERE user = ? AND id IN(${getid}) || admin = ? AND id IN(${getid})ORDER BY id DESC`, [req.session.auth.user,req.session.auth.user])
    res.json({ betsummary: betsummary[0] })
  } else if (req.session.auth.role === 'A1') {
    ...
    const getid = betsid[0].map(item => item.betid)
    const betsummary = await conn.query(`SELECT * FROM betsummary WHERE admin = ? AND id IN(${getid}) || superadmin = ? AND id IN(${getid}) ORDER BY id DESC`, [req.session.auth.user,req.session.auth.user])
    res.json({ betsummary: betsummary[0] })
  } else if (req.session.auth.role === 'B1') {
  ...
  if (cbss[0][0].state === 'Aktif' && betstate === 'Kaybetti') {
    ...
    if (finduser[0][0].autopay) {
      ...
      if (cbs[0].length > 0) {
        ...
        setTimeout(async () => {
          if (cbss[0][0].role === 'C1') {
            const comis = await conn.query('SELECT personcomission FROM users WHERE user = ? LIMIT 1', [betsum.user])
            let getcomission = (drawwon *  Number(comis[0][0].personcomission)) / 100
            let pearn = drawwon - getcomission
            setTimeout(async () => {
              await conn.query('UPDATE betsummary SET adminpay = ? WHERE id = ?', ['yes', betsum.id])
              await conn.query('UPDATE betsummary SET rate = ?, earn = ? WHERE id = ?', [newrate, pearn, betsum.id])
              await conn.query('UPDATE users SET creditremain = creditremain + ' + pearn + ' WHERE user = ? || user = ?', [betsum.user, betsum.admin])
              ope = 'Eklendi'
              des = 'Kazandi'
              insertGameLog(betsum.user, finduser[0][0].user, finduser[0][0].admin, pearn, ope, des)
            }, 10)
          } else {
            await conn.query('UPDATE betsummary SET adminpay = ? WHERE id = ?', ['yes', betsum.id])
        ...
        if (cbss[0][0].role === 'C1') {
          const comis = await conn.query('SELECT personcomission FROM users WHERE user = ? LIMIT 1', [betsum.user])
          let getcomission = (betsum.earn * Number(comis[0][0].personcomission)) / 100
          let pearn = betsum.earn - getcomission
          setTimeout(async () => {
            await conn.query('UPDATE betsummary SET adminpay = ? WHERE id = ?', ['yes', betsum.id])
            await conn.query('UPDATE betsummary SET earn = ? WHERE id = ?', [pearn, betsum.id])
            await conn.query('UPDATE users SET creditremain = creditremain + ' + pearn + ' WHERE user = ? || user = ?', [betsum.user, betsum.admin])
            ope = 'Eklendi'
            des = 'Kazandi'
            insertGameLog(betsum.user, cbss[0][0].admin, finduser[0][0].admin, pearn, ope, des)
          }, 20)
        } else {
          await conn.query('UPDATE betsummary SET adminpay = ? WHERE id = ?', ['yes', betsum.id])
    ...
    if (req.session.auth.autopay) {
      if (cbss[0][0].role === 'C1') {
        await conn.query('UPDATE users SET creditremain = creditremain - ' + oldearn[0][0].earn + ' WHERE user = ? || user = ?', [betsum.user, betsum.admin])
      } else {
        await conn.query('UPDATE users SET creditremain = creditremain - ' + oldearn[0][0].earn + ' WHERE user = ?', [betsum.user])
      }
      await conn.query('UPDATE betsummary SET earn = ? WHERE id = ?', [betsum.baseearn, betsum.id])
      ope = 'Eksildi'
      des = 'Düzeltme'
...
router.put('/paymentbet', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'G1') {
    ...
    if (cbs[0].length > 0) {
      const calcDraw = await conn.query('SELECT rate FROM bets WHERE betid = ? AND state = ?', [betsum.id, 'Kazandi'])
      let newrate = calcDraw[0].reduce(function (acc, item) {
        return acc * item.rate
      }, 1)
      ...
      setTimeout(async () => {
        if (cbss[0][0].role === 'C1') {
          ...
          ope = 'Eklendi'
          des = 'Kazandi'
          insertGameLog(betsum.user, betsum.admin, betsum.superadmin, pearn, ope, des)
        } else {
          await conn.query('UPDATE betsummary SET adminpay = ? WHERE id = ?', ['yes', betsum.id])
          await conn.query('UPDATE betsummary SET rate = ?, earn = ? WHERE id = ?', [newrate, pearn, betsum.id])
          await conn.query('UPDATE users SET creditremain = creditremain + ' + drawwon + ' WHERE user = ?', [betsum.user])
...
router.post('/detailscomissionquickdatefilter', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1') {
    const datefilter = await conn.query('SELECT * FROM betsummary WHERE user =? AND state = ? AND date LIKE "' + req.body.quickdate + '%" ORDER BY id DESC', [req.session.auth.user, 'Kazandi', req.body.datestart, req.body.datefinish])
    res.json({ code: 200, datefilter: datefilter[0] })
  } else {
    if (req.body.userbox === 'Üyeler') {
      const datefilter = await conn.query('SELECT * FROM betsummary WHERE admin =? AND state = ? AND date LIKE "' + req.body.quickdate + '%" ORDER BY id DESC', [req.session.auth.user, 'Kazandi', req.body.datestart, req.body.datefinish])
      res.json({ code: 200, datefilter: datefilter[0] })
    } else {
      const datefilter = await conn.query('SELECT * FROM betsummary WHERE user = ? AND admin = ? AND state = ? AND date LIKE "' + req.body.quickdate + '%" ORDER BY id DESC', [req.body.userbox, req.session.auth.user, 'Kazandi', req.body.datestart, req.body.datefinish])
      res.json({ code: 200, datefilter: datefilter[0] })
    }
  }
}))
...
router.post('/woninfo', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    ...
    if (req.body.userbox === 'Üyeler') {
      ...
      const woninfo = await conn.query('SELECT COUNT(id) AS woncount, SUM(amount) AS wonamount, SUM(earn) AS wonearn FROM betsummary WHERE admin = ? AND state = ? || superadmin = ? AND state = ? ', [req.body.userbox, 'Kazandi', req.body.userbox, 'Kazandi'])
      res.json({ woninfo: woninfo[0][0]})
    }
  }
}))
router.post('/lostinfo', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    ...
    if (req.body.userbox === 'Üyeler') {
      ...
      const lostinfo = await conn.query('SELECT COUNT(id) AS lostcount, SUM(amount) AS lostamount FROM betsummary WHERE admin = ? AND state = ? || superadmin = ? AND state = ? ', [req.body.userbox, 'Kaybetti', req.body.userbox, 'Kaybetti'])
      res.json({ lostinfo: lostinfo[0][0]})
    }
  }
}))
