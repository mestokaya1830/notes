  ...
  methods: {
    getRate(){
      if (localStorage.getItem('set').length > 0) {
        ...
        let r = JSON.parse(ls).find(item => {return item.type === 'sor'})
        if (r) {
          this.newRateText = r.operate +' '+ r.rate.toFixed(2)
        }
        let rl = JSON.parse(ls).find(item => {return item.type === 'sol'})
        if(rl){
    ...
    async updateRate (id) {
      if (this.newRate) {
        this.loader = id
        await axios.post('/api/home/account/soccerupdaterate', {operate:id, newrate: this.newRate, ratelimit:this.rateLimit}).then((result) => {
          this.loader = ''
          this.done = id
          setTimeout(function () {
            this.done = ''
    ...
    async updateLimit (id) {
      if (this.rateLimit) {
        this.loader = id
        await axios.post('/api/home/account/soccerupdateratelimit', {operate:id,ratelimit:this.rateLimit}).then((result) => {
          this.loader = ''
          this.done = id
          setTimeout(function () {
            this.done = ''
