...
export default {
  ...
  methods: {
    ...
    async getGames () {
      await axios.get('/api/home/account/historygames/').then((result) => {
        this.games = result.data.result
      })
    },
    async getOptions () {
      await axios.post('/api/home/account/historyoptions/',{userbox: this.userBox}).then((result) => {
    ...
    async getBets (event, id) {
      this.bets = {}
      this.preloader = true
      await axios.post('/api/home/account/historybet/',{userbox:this.userBox, value:event.target.value, column:id}).then((result) => {
    ...
    async getBetsMatches () {
      if (this.matchesSearch) {
        this.bets = {}
        this.preloader = true
        await axios.post('/api/home/account/historymatches/',{userbox:this.userBox, value:this.matchesSearch}).then((result) => {
          if (result.data.result.length >  0) {
            this.notfound = ''
            this.preloader = false
            this.bets = result.data.result
