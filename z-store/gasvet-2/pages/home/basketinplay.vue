  ...
  methods: {
    async getLive() {
      await axios.get("/api/home/basketinplay").then((result) => {
        ...
        this.suspendBet = result.data.map(item => {
          return {
    ...
    async liveOddsFunc(){
      ...
      await axios.get(`/api/home/inplayodds/${this.liveEvents.Id}`).then((result) => {
        ...
        if (localStorage.getItem('set')) {
          ...
          let lt = ls.find(item => item.type === 'ball')
          if (lt) {
            newlimit = lt.rate
          }
          mid = ls.map(item => {
          ...
          let filteredodds = result.data[0].Markets.filter(item1 => { return !mid.some(item2 => (item2.filterid === item1.templateId))})
