...
export default {
  ...
  mounted (){
    if(localStorage.getItem('set') !== null){
      const br = JSON.parse(localStorage.getItem('set')).find(item => item.filterid === 3)
      if(!br){
        this.getLeagues()
  ...
  methods: {
    async getLeagues(){
      ...
      await axios.get('/api/home/basketfixture').then((result)=>{
        if (result.data) {
          const filternow = result.data.filter(item => moment(item.startdate).tz('Europe/Istanbul').format('YYYY MM DD HH:mm') > moment().format('YYYY MM DD HH:mm'))
          if (localStorage.getItem('set')) {
            const ls = JSON.parse(localStorage.getItem('set'))
            lid = ls.map(item => {
              if (item && item.type === 'ba') {
                return {filterid:item.filterid}
              }else{return 0}
            })
            final = filternow.filter(item1 => { return !lid.some(item2 => (item2.filterid === item1.leagueid))})
          }else{
            final = filternow
          }
          ...
          let favtemp = final.filter(item => item.RegionId === 31 || item.RegionId === 37 || item.RegionId === 14 || item.RegionId === 36 || item.RegionId === 17 || item.RegionId === 16 || item.RegionId === 20)
    ...
    addSlip (rate, option, sublist, games, handikap, total) {
      ...
      const st = moment(sublist.startdate).tz('Europe/Istanbul').format('YYYY-MM-DD HH:mm')
