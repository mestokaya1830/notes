        ...
        <ul>
          <li class="title-ligth">
            <div class="blinks">
              <span class="bitems">
                <div>
                  <input v-model="searchBox" type="text" class="bet-inp" disabled>
        ...
        <ul class="bet-summary-con po-r">
          <div class="list-bet"  v-for="(list,index) in filterBy(betSummary, searchBox)" :key="index" :class="list.state">
            <li class="blinks"  @click="getBets($event, index, list)">
              ...
              <span v-if="checkAdmin && !checkSubadmin" class="bitems-list bowner">{{ list.user | capitalize}}</span>
            ...
            <span class="bitems-list bstate">
              <div class="payment-con" v-if="checkMember && list.state === 'Kazandi'">
              ...
              <div class="payment-con" v-if="checkAdmin && !checkSubadmin && list.state === 'Kazandi'">
              ...
              <i class="fas fa-trash-alt bet-cancel" v-if="list.state ==='Aktif' && checkAdmin && !checkSubadmin" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="checkMember && cancelTime(list.date) < $store.state.auth.usercancelbet  && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="checkSubmember && cancelTime(list.date) < $store.state.auth.personcancelbet && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
            </span>
          </div>
                ...
                <div class="bet-con">
                  <ul>
                    <li class="title-ligth">
                      ...
                      <span v-if="checkAdmin && !checkSubadmin || checkSuperadmin || checkBoss" class="bitems bstate">DURUM</span>
                    </li>
                  </ul>
                  <ul class="bet-details po-r">
                    <li class="list-bet details-list"  v-for="(list,index) in bets" :key="index" :class="{innerAktif:list.state === 'Aktif',Kazandi:list.state === 'Kazandi', Kaybetti:list.state === 'Kaybetti', iade:list.state === 'iade',iptalrepeat:list.betid === cancelInner, iptal:list.state === 'iptal'}">
                      ...
                      <span v-if="checkAdmin && !checkSubadmin || checkSuperadmin || checkBoss && list.code !== isBetLoader" class="bitems-list bstate">
