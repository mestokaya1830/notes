      ...
      <form class="reg-form">
        ...
        <div class="resetaccount-items">
          ...
          <span>
            ...
            <span class="subcredit cl-r" v-if="$store.state.auth.role === 'G1'  && role && role.role === 'G1' && subCreditInfo">({{subCreditInfo.subcredit | currency('', 0)}})</span>
            <span class="subcredit cl-r" v-if="$store.state.auth.role === 'A2' && role && role.role === 'G1' && subCreditInfo">({{subCreditInfo.subcredit | currency('', 0)}})</span>
          </span>
        </div>
        <section v-if="checkEarn && role.role !== 'C1'">
          <div class="resetaccount-items">
...
export default {
  ...
  computed: {
    ...
    checkEarn () {
      if (this.subCreditInfo) {
        if (this.creditInfo.credit > (this.creditInfo.creditremain + this.subCreditInfo.subcredit)) {
          return true
        } else {
          return false
        }
      ...
      try {
        if (this.$store.state.auth.role === 'A2' && this.userBox === 'Üyeler' || this.$store.state.auth.role === 'G1' && this.userBox === 'Üyeler') {
          ...
          Promise.all([creditinfo, subcredit, comission, total]).then(result => {
            this.creditInfo = result[0].data.creditinfo[0]
