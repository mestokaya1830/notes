          ...
          <details :open="isLeagueOpen" class="odd-details cp" v-for="(list, index) in leagues" :key="index">
            ...
            <div class="list odd-list" v-for="(sublist, index) in list" :key="index">
              ...
              <div class="odd-right-list-con">
               ...
               <span class="ottli ottli-last" @click="getOdds($event,sublist.fixtureid)"><i class="fas fa-plus-square sso cp cl-f"/></span>
              </div>
            </div>
          </details>
        </div>
...
export default {
  ...
  methods: {
      ...
      await axios.get('/api/home/soccerfixture').then((result)=>{
        if (result.data.length > 0) {
          ...
          if (localStorage.getItem('set')) {
            ...
            lid = ls.map(item => {
              if (item && item.type === 'so') {
                return {filterid:item.filterid}
              }else{return 0}
            })
            final = result.data.filter(item1 => { return !lid.some(item2 => (item2.filterid === item1.leagueid))})
          }else{
            final = result.data
          }
          this.leagues = _.groupBy(final, 'leagueid')
          this.search = final
          this.leaguesLength = final.length
          this.isLoader = false
          let favtemp = final.filter(item => item.RegionId === 31 || item.RegionId === 37 || item.RegionId === 14 || item.RegionId === 36 || item.RegionId === 17 || item.RegionId === 16 || item.RegionId === 20 || item.RegionId === 28)
          this.favorites = _.groupBy(favtemp, 'RegionId')
          let fixtureid = final.map(item => item.fixtureid)
    ...
    getFav (id) {
      let test = this.search.filter(item => item.RegionId === id)
    ...
    searchFilter () {
      let searchArray = this.search.filter(item => item.HomeTeam.toLowerCase().includes(this.searchBox.toLowerCase()) || item.AwayTeam.toLowerCase().includes(this.searchBox.toLowerCase()) || item.fixtureid.toString().slice(-3).startsWith(this.searchBox))
