  ...
  methods: {
    async getLive() {
      ...
      await axios.get("/api/home/soccerinplay").then((result) => {
        let mainfilter = result.data.filter(item =>  !item.LeagueName.includes('eSoccer') && !item.LeagueName.includes('Price Boost') && !item.LeagueName.includes('Enhanced Accas'))
        ...
        final.forEach(item => {
          if (item.Scoreboard.period === '1H') {
            let x = new Date(moment.utc(item.Scoreboard.timer.base).tz('Europe/Istanbul').format('YYYY-MM-DD HH:mm:ss'))
    ...
    async liveOddsFunc(){
      ...
      await axios.get(`/api/home/inplayodds/${this.liveEvents.Id}`).then((result) => {
        ...
        if (localStorage.getItem('set')) {
          let ls = JSON.parse(localStorage.getItem('set'))
          let lt = ls.find(item => item.type === 'soll')
          if (lt) {
            newlimit = lt.rate
          }
          mid = ls.map(item => {
          ...
          let filteredodds = defaultfilter.filter(item1 => { return !mid.some(item2 => (item2.filterid === item1.templateId))})
        ...
        if (result.data[0].Scoreboard.period && result.data[0].Scoreboard.period === '1H') {
            let x = new Date(moment.utc(result.data[0].Scoreboard.timer.base).tz('Europe/Istanbul').format('YYYY-MM-DD HH:mm:ss'))
    ...
    searchFilter () {
      let searchArray = this.search.filter(item => item.HomeTeam.toLowerCase().includes(this.searchBox.toLowerCase()) || item.AwayTeam.toLowerCase().includes(this.searchBox.toLowerCase()))
    ...
    addSlip (rate,option,game,tid) {
      let match = this.liveEvents.HomeTeam +' - '+ this.liveEvents.AwayTeam
