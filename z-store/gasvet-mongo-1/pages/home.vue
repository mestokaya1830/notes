      ...
      <div class="home-top-right">
        <div class="home-top-inside htir">
          ...
          <div class="hhli">
            ...
            <span class="cl-g">{{$store.state.betinfo.creditremain | currency('', 0)}} {{ $store.state.betinfo.creditremainsum | currency('', 2)}}</span>
  ...
  methods:{
    ...
    checkRole(){
      if (this.$store.state.auth) {
        if (this.$store.state.auth.role !== 'B4' && this.$store.state.auth.role !== 'S3') {
          return true
        }
      }
    },
