  ...
  <div v-if="checkRole" class="pages">
    ...
    <ul>
      <li class="title-ligth">
        ...
        <span v-if="checkA2" class="uitems bitems-title">LİMİT</span>
        <span v-if="checkA2 || checkG1" class="uitems bitems-title">BAKİYE</span>
        <span v-if="checkB4 || checkS3" class="uitems bitems-title">ÜYE SAYISI</span>
    ...
    <ul class="bet-summary-con-admin">
      <li v-for="list in users" :key="list._user">
        <nuxt-link class="list" :to="'/home/account/users/' + list.user">
          ...
          <span v-if="checkA2" class="uitems-list">{{ list.credit | currency('', 0) }}</span>
          <span v-if="checkA2 || checkG1" class="uitems-list">
            {{ list.creditremain | currency('', 0) }}
            <div v-if="list.role === 'G1'">
              (<span class="cl-r subcredit">{{list.subcredit | currency('', 0)}}</span>)
            </div>
          </span>
          <span v-if="checkB4 || checkS3" class="uitems-list">{{ list.userlimit }} / {{ list.userlimit - list.usedlimit }}</span>
