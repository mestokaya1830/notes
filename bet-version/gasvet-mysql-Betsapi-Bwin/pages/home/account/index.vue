<template>
 <div class="pages" v-if="$store.state.auth.role === 'S1'">Hello</div>
  <div class="pages" v-else>
    <div>
      <header class="title-2">
        <div class="quick-report-con">
          <span class="bitems bstate" v-if="!checkS1 && !checkB1">
          ...
          <div class="date-con" v-if="!checkS1 && !checkB1">
          ...
          <div class="input-con" v-if="!checkS1 && !checkB1">
      ...
      <div class="bet-con">
        <ul>
          <li class="title-ligth">
            ...
            <span class="bitems bstate" v-if="checkB1">AKTİFLER</span>
        ...
        <ul class="bet-summary-con po-r">
          <div class="list-bet"  v-for="(list,index) in betSummary" :key="index" :class="list.state">
            <li class="blinks"  @click="getBets($event, index, list)">
              ...
              <span v-if="checkG1 || checkA1" class="bitems-list bowner">{{ list.user | capitalize}}</span>
              <span v-if="checkB1" class="bitems-list bowner">{{ list.admin | capitalize}}</span>
            ...
            <span class="bitems-list bstate">
              <div class="payment-con" v-if="checkA1 && list.state === 'Kazandi'">
              ...
              <div class="payment-con" v-if="checkG1 && list.state === 'Kazandi'">
                ...
                <input type="button" class="bitems-button bg-b" v-if="list.adminpay === 'yes' && list.userpay === 'no'" value="ÖDE" @click="paymentBet(list.id,'',$event)"/>
              ...
              <i class="fas fa-bolt" v-if="list.state === 'iptal'" @click="returnBet(list.id, index, list.amount, list.user)" />
              <i class="fas fa-retweet" v-if="list.state === 'iade'" />
              <i class="fas fa-balance-scale" v-if="list.state === 'Aktif'" />
              <i class="fas fa-trash-alt bet-cancel" v-if="list.state === 'Aktif' && checkA1" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="cancelTime(list.date) < $store.state.auth.usercancelbet &&  checkU1 && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="cancelTime(list.date) < $store.state.auth.usercancelbet &&  checkG1 && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="cancelTime(list.date) < $store.state.auth.personcancelbet && checkC1 && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
            </span>
          </div>
              ...
              <div  class="bet-details-inside">
                <header class="title-3">
                  ...
                  <span>Kupon Detay</span>
                    ...
                    <span class="ml20 cl-y cp" @click="removeInnerBet()"  v-if="checkA1 && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">iptal</span>
                ...
                <div class="bet-con">
                  ...
                  <ul class="bet-details po-r">
                    <li class="list-bet details-list"  v-for="(list,index) in bets" :key="index" :class="{innerAktif:list.state === 'Aktif',Kazandi:list.state === 'Kazandi', Kaybetti:list.state === 'Kaybetti', iade:list.state === 'iade',iptalrepeat:list.betid === cancelInner, iptal:list.state === 'iptal'}">
                      ...
                      <span v-if="checkA1 || checkB1 && list.code !== isBetLoader" class="bitems-list bstate">
...
export default {
  ...
  methods: {
    ...
    async removeBet(index){
      ...
      if (this.$store.state.auth.role !== 'A1') {
        if (this.$store.state.limit.maxkis !== 0 && this.$store.state.limit.maxkisremain === this.$store.state.limit.maxkis) {
    ...
    async removeInnerBet(){
      ...
      await axios.put('/api/home/account/cancelbet', {id: this.betSum.id, amount: this.betSum.amount, user: this.betSum.user}).then((result) => {
        if (result.data.code === 200) {
          this.betSummary.splice(this.removeIndex,1)
          this.betInfo()
          this.removeIndex = null
    ...
    async returnBet(id,index,amount,user){
      await axios.put('/api/home/account/returnbet', {id: id, amount: amount, user: user}).then((result) => {
        if (result.data.code === 200) {
