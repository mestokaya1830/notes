<template>
  <div class="pages">
    <header class="title-2" id="content">
      <span>FUTBOL CANLI BAHİS DÜZENLE</span>
    ...
    <ul class="main-set-list">
      ...
      <li v-for="(list, index) in markets" :key="index" class="fix-li">
        <div class="fix-1">
          ...
          <input v-model="list[$store.state.auth.user]" type="checkbox" class="check-inp ml50" @change="updateLiveMarkets(list[$store.state.auth.user], list.mid)">
...
export default {
  ...
  methods: {
    ...
    async getMarkets () {
      this.loader = true
      await axios.get('/api/home/account/markets/soccerinplaymarkets').then((result) => {
        this.markets = result.data.data
        this.marketsLength = result.data.data.length
        this.loader = false
      })
    },
    async updateLiveMarkets (state, mid) {
      this.setloader = mid
      await axios.post('/api/home/account/soccerinplaymarketsupdate', {state:state, mid: mid}).then((result) => {
        this.done = mid
        this.setloader = ''
        setTimeout(function () {
          this.done = ''
