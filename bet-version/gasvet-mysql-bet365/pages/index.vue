      ...
      <form class="login-form" @submit.prevent="login">
        ...
        <div class="login-center">
          <div class="login-inputs-con">
            ...
            <input type="text" v-model="user" @focus="removeError()" class="login-inputs" placeholder="Üye Adı" required oninvalid="this.setCustomValidity('Üye alanı boş bırakılamaz!')" oninput="setCustomValidity('')" tabindex='1'>
          </div>
          <div class="login-inputs-con">
        ...
        <footer class="login-footer">
          <input type="submit" v-if="btnLogin" value="Giriş" class="alert btn-alert cl-d">
...
export default {
  ...
  mounted () {
    if (this.$store.state.checkauth === 'true') {
      ...
      if (navigator.onLine === true){
        ...
        await axios.post('/api/login', { user: this.user, pass: this.pass }).then((result) => {
          if (result.data.code === 200) {
            ...
            setTimeout(() => {
              this.$store.commit('setCheckAuth', 'true')
              this.$router.push('/home')
              this.loader = false
              axios.get('/api/home/soccer').then((result)=>{
                axios.get('/api/home/basketball').then((result)=>{
                  console.log('Done...')
                })
              })
            }, 1000) 
