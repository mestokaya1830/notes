        ...
        <ul class="bet-summary-con po-r">
          <div class="list-bet"  v-for="(list,index) in filterBy(betSummary, searchBox)" :key="index" :class="list.state">
            ...
            <span class="bitems-list bstate">
              ...
              <i class="fas fa-trash-alt bet-cancel" v-if="list.state ==='Aktif' && checkAdmin" @click="cancelBet($event, list.id, list.amount, list.user,index)" />
...
export default {
  ...
  methods: {
    ...
    async getBetSummary () {
      this.isLoader = true
      await axios.post('/api/home/account/betsummarysingle', {userbox: this.userBox}).then((result) => {
