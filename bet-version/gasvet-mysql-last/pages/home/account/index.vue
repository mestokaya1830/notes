    ...
    <div>
      <header class="title-2">
        <div class="quick-report-con">
          <span class="bitems bstate" v-if="!checkSuperadmin && !checkBoss">
          ...
          <div class="date-con" v-if="!checkSuperadmin && !checkBoss">
          ...
          <div class="input-con" v-if="!checkSuperadmin && !checkBoss">
          ...
          <li class="title-ligth">
            <div class="blinks">
              <span class="bitems betid">
                <div>
                  <input v-model="searchBox" type="number"  @keyup ="getBetsummaryCode()" class="bet-inp" placeholder="Ara...">
                  <span class="searchbox-border"/>
                </div>
              </span>
              <span class="bitems cp">
                <span>
                  <span v-if="checkAdmin || checkSubmember">SAHİP</span>
                  <span v-else>
        ...
        <ul class="bet-summary-con po-r">
          <div class="list-bet"  v-for="(list,index) in betSummary" :key="index" :class="list.state">
            <li class="blinks"  @click="getBets($event, index, list)">
              <span class="bno-inside bg-b">{{ list.id }}</span>
              <span v-if="checkSubmember || checkAdmin" class="bitems-list bowner" :class="{filterBetClass: sortName === 'owner'}">{{ list.owner }}</span>
              <span v-if="checkSubadmin || checkAdmin" class="bitems-list bowner">{{ list.user | capitalize}}</span>
              <span v-if="checkBoss" class="bitems-list bowner">{{ list.admin | capitalize}}</span>
            ...
            <span class="bitems-list bstate">
              <div class="payment-con" v-if="checkAdmin && list.state === 'Kazandi'">
              ...
              <div class="payment-con" v-if="checkSubadmin && list.state === 'Kazandi'">
                <input type="button" class="bitems-button bg-y" v-if="list.adminpay === 'no'" value="BEKLİYOR" disabled/>
                <input type="button" class="bitems-button bg-b" v-if="list.adminpay === 'yes' && list.userpay === 'no'" value="ÖDE" @click="paymentBet(list.betid,'',$event)"/>
              ...
              <div class="payment-con" v-if="checkSubmember && list.state === 'Kazandi'">
              ...
              <i class="fas fa-trash-alt bet-cancel" v-if="list.state === 'Aktif' && checkAdmin" @click="cancelBet($event, list, index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="cancelTime(list.date) < $store.state.auth.usercancelbet &&  checkAdmin && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list, index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="cancelTime(list.date) < $store.state.auth.usercancelbet &&  checkSubadmin && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list, index)" />
              <i class="fas fa-trash-alt bet-cancel" v-if="cancelTime(list.date) < $store.state.auth.personcancelbet && checkSubmember && list.stream === 'Bülten' && list.state === 'Aktif'" @click="cancelBet($event, list, index)" />
            </span>
          </div>
            ...
            <div class="bet-details-con">
              <div  class="bet-details-inside">
                <header class="title-3">
                  <div>
                  <span>KUPON NO:</span> <span class="cpno">{{ bets[0].betid }}</span>
                    <i class="fas fa-print cp fa-lg ml20 cl-y" v-print="'#printMe'"/>
                    <span class="ml20 cl-y cp" @click="removeInnerBet()"  v-if="checkAdmin && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">iptal</span>
                    <span class="ml20 cl-y cp" @click="repeatBets(bets)" v-if="checkSubadmin &&  repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">Tekrar Yatır</span>
                    <span class="ml20 cl-y cp" @click="removeInnerBet()"  v-if="checkSubadmin && (cancelTime(betSum.date) < $store.state.auth.usercancelbet) && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">iptal</span>
                    <span class="ml20 cl-y cp" @click="repeatBets(bets)" v-if="checkAdmin && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">Tekrar Yatır</span>
                    <span class="ml20 cl-y cp" @click="removeInnerBet()"  v-if="checkAdmin && (cancelTime(betSum.date) < $store.state.auth.usercancelbet) && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">iptal</span>
                    <span class="ml20 cl-y cp" @click="repeatBets(bets)" v-if="checkSubmember && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">Tekrar Yatır</span>
                    <span class="ml20 cl-y cp" @click="removeInnerBet()"  v-if="checkSubmember && (cancelTime(betSum.date) < $store.state.auth.personcancelbet) && repeatTime && betSum.state === 'Aktif' && betSum.stream === 'Bülten'">iptal</span>
                  </div>
                ...
                <div class="bet-con">
                  <ul>
                    <li class="title-ligth">
                      ...
                      <span v-if="checkAdmin || checkBoss" class="bitems bstate">DURUM</span>
                    </li>
                  </ul>
                  <ul class="bet-details po-r">
                    <li class="list-bet details-list"  v-for="(list,index) in bets" :key="index" :class="{Aktif:list.state === 'Aktif',Kazandi:list.state === 'Kazandi', Kaybetti:list.state === 'Kaybetti', iade:list.state === 'iade',iptalrepeat:list.betid === cancelInner, iptal:list.state === 'iptal'}">
                      ...
                      <span v-if="checkAdmin || checkBoss && list.code !== isBetLoader" class="bitems-list bstate">
      ...
      <div v-if="isWarningOpen" class="dialog warning warning-bet" :style="{top:top}">
        <header class="cancelbet-header"><span class="cancelbet-no">{{betSum.id}}</span></header>
...
export default {
  ...
  methods: {
    ...
    async getBetSummary () {
      ...
      await axios.post('/api/home/account/betsummary', {userbox: this.userBox, statebox: this.stateBox,date:today}).then((result) => {
    ...
    async paymentBet(list, event){
      await axios.put('/api/home/account/paymentbet', {id: list.betid, list:list}).then((result) => {
        if (result.data.code === 200) {
    ...
    async removeInnerBet(){
      this.isWarningOpen = false
      this.cancelInner = this.betSum.id
      await axios.put('/api/home/account/cancelbet', {betsum: this.betSum}).then((result) => {
        if (result.data.code === 200) {
          this.betSummary.splice(this.removeIndex,1)
          this.betInfo()
    ...
    getTotal(value){
      const wonactive = value.filter(item => item.state === 'Aktif' || item.state === 'Kazandi')
