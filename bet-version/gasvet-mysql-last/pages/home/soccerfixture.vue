    ...
    async getLeagues(){
      ...
      await axios.get('/api/home/soccerfixture').then((result)=>{
        if (result.data.length > 0) {
          ...
          this.isLoader = false
          const favtemp = final.filter(item => item.RegionId === 31 || item.RegionId === 37 || item.RegionId === 14 || item.RegionId === 36 || item.RegionId === 17 || item.RegionId === 16 || item.RegionId === 20 || item.RegionId === 28)
