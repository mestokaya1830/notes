...
//   try {
//     (async function () {
//       let result = request('GET', `https://api.betsapi.com/v1/bwin/inplay?token=23479-hVapIXdy0gkZIo&sport_id[]=4`,{json: {results: 'results'}})
...
//           })
//         }
//         let htou15 = await conn.query('SELECT id, betid, scoreid, matches, games, options FROM bets WHERE state = ? AND games = ?',['Aktif', 'İlk Yarı Toplam Gol Alt/Üst 1.5'])
//         if (htou15.length > 0) {
//           halftimescors.forEach(item2 => {
//             htou15[0].forEach(item => {
...
//           })
//         }
//         let hthomeou15 = await conn.query('SELECT id, betid, scoreid, matches, games, options FROM bets WHERE state = ? AND games = ?',['Aktif', 'İlk Yarı Evsahibi Toplam Gol Alt/Üst 1.5'])
//         if (hthomeou15.length > 0) {
//           halftimescors.forEach(item2 => {
//             hthomeou15[0].forEach(item => {
//               if (item2.matches.trim() === item.matches.trim() && item.options.trim() === 'Üst 1.5') {
//                 if (Number(item2.score.charAt(0)) > 0) {
//                   conn.query('UPDATE bets set state = ? WHERE id IN("'+item.id+'")',['Kazandi'])
//                   conn.query('UPDATE betsummary SET betscountremain = betscountremain + 1 WHERE id = ?', [item.betid])
...
//           })
//         }
//         let htawayou15 = await conn.query('SELECT id, betid, scoreid, matches, games, options FROM bets WHERE state = ? AND games = ?',['Aktif', 'İlk Yarı Deplasman Toplam Gol Alt/Üst 1.5'])
//         if (htawayou15.length > 0) {
//           halftimescors.forEach(item2 => {
//             htawayou15[0].forEach(item => {
//               if (item2.matches.trim() === item.matches.trim() && item.options.trim() === 'Üst 1.5') {
//                 if (Number(item2.score.charAt(2)) > 0) {
//                   conn.query('UPDATE bets set state = ? WHERE id IN("'+item.id+'")',['Kazandi'])
//                   conn.query('UPDATE betsummary SET betscountremain = betscountremain + 1 WHERE id = ?', [item.betid])
...
//           })
//         }
//         let ou15 = await conn.query('SELECT id, betid, scoreid, matches, games, options FROM bets WHERE state = ? AND games = ?',['Aktif', 'Toplam Gol Alt/Üst 1.5'])
//         if (ou15.length > 0) {
//           finishedscors.forEach(item2 => {
//             ou15[0].forEach(item => {
//               if (item2.matches.trim() === item.matches.trim() && item.options.trim() === 'Üst 1.5') {
...
router.post('/addbet', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'G1' || req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    ...
    let betSummary = new Betsummaries({
      sumid: ides.sumid,
    ...
    const checkcredit = await Users.findOne({user: req.session.auth.user}, 'credit creditremain').exec()
    if (newSummary.amount > checkcredit.creditremain) {
      ...
      const result = await Users.findOne({user: req.session.auth.user}, 'user admin superadmin creditremain').exec()
      let gamelog = new Gamelogs({
        gameid: ides.sumid,
        user: result.user,
        admin: result.admin,
...
router.put('/returnbet', wrapAsync(async (req, res) => {
  const ccr = await Users.findOne({user: req.body.betsum.user}, 'creditremain').exec()
  if (req.body.betsum.amount > ccr.creditremain) {
    ...
    await Users.updateOne({user: req.body.betsum.user}, {$inc: {creditremain: - req.body.betsum.amount}})
    const lcr = await Users.findOne({user: req.body.betsum.user},'creditremain').exec()
    let gamelog = new Gamelogs({
      gameid: req.body.betsum.sumid,
      user: req.body.betsum.user,
      admin: req.body.betsum.admin,
...
router.get('/lastcoupon', wrapAsync(async (req, res) => {
  const result = await Betsummaries.findOne({user: req.session.auth.user}).exec()
  if (result) {
    const result2 = await Bets.find({id: result.maxid, stream: 'Bülten'}, 'code matches games options rate scoreid eventtime').exec()
    if (result2.length > 0) {
      res.json(result2[0])
    } else {
      res.json({ status: 401 })
...
router.post('/betsummary', wrapAsync(async (req, res) => {
  ...
  let end = moment().format("YYYY-MM-DD 24:00:00")
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    if (req.body.statebox === 'Aktif') {
      const betsummary = await Betsummaries.find({user: req.session.auth.user, state: 'Aktif'}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    } else {
      if (req.body.statebox === 'Hepsi') {
        const betsummary = await Betsummaries.find({user: req.session.auth.user, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}}).sort({_id: -1}).exec()
        res.json({ betsummary: betsummary })
      } else {
        const betsummary = await Betsummaries.find({user: req.session.auth.user, date: {$gte: start, $lt: end}, state: req.body.statebox}).sort({_id: -1}).exec()
        res.json({ betsummary: betsummary })
      }
    }
  }else if (req.session.auth.role === 'G1') {
    if (req.body.userbox === 'Üyeler') {
      if (req.body.statebox === 'Aktif') {
        const betsummary = await Betsummaries.find({$or:[{user: req.session.auth.user, state: 'Aktif'},{admin: req.session.auth.user, state: 'Aktif'}]}).sort({sumid: -1}).exec()
        res.json({ betsummary: betsummary })
      } else {
        if (req.body.statebox === 'Hepsi') {
          const betsummary = await Betsummaries.find({$or:[{user: req.session.auth.user, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}},{admin: req.session.auth.user, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}}]}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        } else {
          const betsummary = await Betsummaries.find({$or:[{user: req.session.auth.user, date: {$gte: start, $lt: end}, state:req.body.statebox},{admin: req.session.auth.user, date: {$gte: start, $lt: end}, state:req.body.statebox}]}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        }
      }
    }else{
      if (req.body.statebox === 'Aktif') {
        const betsummary = await Betsummaries.find({user: req.body.userbox, state: 'Aktif'}).sort({sumid: -1}).exec()
        res.json({ betsummary: betsummary })
      } else {
        if (req.body.statebox === 'Hepsi') {
          const betsummary = await Betsummaries.find({user: req.body.userbox, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        } else {
          const betsummary = await Betsummaries.find({user: req.body.userbox, state: req.body.statebox}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        }
      }
    }
    ...
    if (req.body.userbox === 'Üyeler') {
      if (req.body.statebox === 'Aktif') {
        const betsummary = await Betsummaries.find({$or:[{admin: req.session.auth.user, state: 'Aktif'},{superadmin: req.session.auth.user, state: 'Aktif'}]}).sort({sumid: -1}).exec()
        res.json({ betsummary: betsummary })
      } else {
        if (req.body.statebox === 'Hepsi') {
          const betsummary = await Betsummaries.find({$or:[{admin: req.session.auth.user, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}},{superadmin: req.session.auth.user, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}}]}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        } else {
          const betsummary = await Betsummaries.find({$or:[{admin: req.session.auth.user, date: {$gte: start, $lt: end}, state:req.body.statebox},{superadmin: req.session.auth.user, date: {$gte: start, $lt: end}, state:req.body.statebox}]}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        }
      }
    }else{
      if (req.body.statebox === 'Aktif') {
        const betsummary = await Betsummaries.find({$or:[{user:req.body.userbox, state: 'Aktif'},{admin:req.body.userbox, state: 'Aktif'}]}).sort({sumid: -1}).exec()
        res.json({ betsummary: betsummary })
      } else {
        if (req.body.statebox === 'Hepsi') {
          const betsummary = await Betsummaries.find({$or:[{user:req.body.userbox, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}},{admin:req.body.userbox, date: {$gte: start, $lt: end}, state:{$ne:'iptal'}}]}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        } else {
          const betsummary = await Betsummaries.find({$or:[{user:req.body.userbox, date: {$gte: start, $lt: end}, state:req.body.statebox},{admin:req.body.userbox, date: {$gte: start, $lt: end}, state:req.body.statebox}]}).sort({sumid: -1}).exec()
          res.json({ betsummary: betsummary })
        }
      }
    }
  }else if (req.session.auth.role === 'B1') {
    const betsummary = await Betsummaries.find({state:{$ne:'iptal'}}).sort({_id: -1}).exec()
    res.json({ betsummary: betsummary })
  }
}))
router.get('/betsummary/:id', wrapAsync(async (req, res) => {
  betref = req.params.id
  const bets = await Bets.find({sumid: req.params.id}).sort({_id: -1}).exec()
  res.json({ bets: bets })
}))
router.post('/betsummary/quickdatefilter', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    if (req.body.statebox === 'Hepsi') {
      const datefilter = await Betsummaries.find({user: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}}).sort({_id: -1}).exec()
      res.json({ code: 200, datefilter: datefilter })
    } else {
      const datefilter = await Betsummaries.find({user: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox}).sort({_id: -1}).exec()
      res.json({ code: 200, datefilter: datefilter })
    }
  } else if (req.session.auth.role === 'G1'){
    if (req.body.userbox === 'Üyeler') {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await Betsummaries.find({$or :[{user: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}},{admin: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await Betsummaries.find({$or:[{user: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox},{admin: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    } else {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await Betsummaries.find({user: req.body.userbox, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await Betsummaries.find({user: req.body.userbox, date:  {$gte: req.body.start, $lt: req.body.end}, state: req.body.statebox}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    }
  } else {
    if (req.body.userbox === 'Üyeler') {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await Betsummaries.find({$or :[{admin: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}},{superadmin: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await Betsummaries.find({$or:[{admin: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox},{superadmin: req.session.auth.user, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    } else {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await Betsummaries.find({$or :[{user: req.body.userbox, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}},{admin: req.body.userbox, date:  {$gte: req.body.start, $lt: req.body.end}, state:{$ne: 'iptal'}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await Betsummaries.find({$or:[{user: req.body.userbox, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox},{admin: req.body.userbox, date:  {$gte: req.body.start, $lt: req.body.end}, state:req.body.statebox}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    }
  }
...
router.post('/betsummary/datefilter', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    if (req.body.statebox === 'Hepsi') {
      const datefilter = await Betsummaries.find({user: req.session.auth.user, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
      res.json({ code: 200, datefilter: datefilter })
    } else {
      const datefilter = await Betsummaries.find({user: req.session.auth.user, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
      res.json({ code: 200, datefilter: datefilter })
    }
  } else if(req.session.auth.role === 'G1'){
    if (req.body.userbox === 'Üyeler') {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await Betsummaries.find({$or: [{user: req.session.auth.user, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}, {admin: req.session.auth.user, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await Betsummaries.find({$or: [{user: req.session.auth.user, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}, {admin: req.session.auth.user, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    } else {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await Betsummaries.find({user: req.session.auth.user, state:{$ne:'iptal'}, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await Betsummaries.find({user: req.session.auth.user, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
    ...
    if (req.body.userbox === 'Üyeler') {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await await Betsummaries.find({$or: [{admin: req.session.auth.user, state: {$ne:'iptal'}, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}, {superadmin: req.session.auth.user, state: {$ne:'iptal'}, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await await Betsummaries.find({$or: [{admin: req.session.auth.user, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}, {superadmin: req.session.auth.user, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    } else {
      if (req.body.statebox === 'Hepsi') {
        const datefilter = await await Betsummaries.find({$or: [{user: req.body.userbox, state: {$ne:'iptal'}, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}, {admin: req.body.userbox, state: {$ne:'iptal'}, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      } else {
        const datefilter = await await Betsummaries.find({$or: [{user: req.body.userbox, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}, {admin: req.body.userbox, state: req.body.statebox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}]}).sort({_id: -1}).exec()
        res.json({ code: 200, datefilter: datefilter })
      }
    }
  }
...
router.post('/paycupons', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'G1') {
    const betsummary = await Betsummaries.find({user: req.session.auth.user, adminpay: 'no', state:'Kazandi'}).sort({_id: -1}).exec()
    res.json({ betsummary: betsummary })
  } else if (req.session.auth.role === 'A2') {
    if (req.body.userbox === 'Üyeler') {
      const betsummary = await Betsummaries.find({$or:[{admin: req.session.auth.user, adminpay: 'no', state:'Kazandi'},{superadmin: req.session.auth.user, adminpay: 'no', state:'Kazandi'}]}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    } else {
      const betsummary = await Betsummaries.find({$or:[{user: req.body.userbox, adminpay: 'no', state:'Kazandi'},{admin: req.body.userbox, adminpay: 'no', state:'Kazandi'}]}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    }
  }
}))
router.post('/betsummarysingle', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    ...
    if (req.body.userbox === 'Üyeler') {
      const betsummary = await Betsummaries.find({$or:[{user: req.session.auth.user, state:'Aktif'},{admin: req.session.auth.user, state:'Aktif'}]}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    } else {
      const betsummary = await Betsummaries.find({user: req.session.auth.user, state:'Aktif'}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    ...
    if (req.body.userbox === 'Üyeler') {
      const betsummary = await Betsummaries.find({$or:[{admin: req.session.auth.user, state:'Aktif'},{superadmin: req.session.auth.user, state:'Aktif'}]}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    } else {
      const betsummary = await Betsummaries.find({$or:[{user: req.session.auth.user, state:'Aktif'},{admin: req.session.auth.user, state:'Aktif'}]}).sort({_id: -1}).exec()
      res.json({ betsummary: betsummary })
    ...
    const betsummary = await Betsummaries.find({state:'Aktif'}).sort({_id: -1}).exec()
    res.json({ betsummary: betsummary })
  }
}))
router.post('/betsummarynextgoals', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    const betsid = await Bets.query.aggregate([
    ...
    const betsummary = await Bets.find({id:{$in: getid}}).sort({_id: -1}).exec()
    res.json({ betsummary: betsummary })
  } else if (req.session.auth.role === 'G1') {
    const betsid = await Bets.query.aggregate([
...
router.post('/betsummarycode', wrapAsync(async (req, res) => {
  betref = req.params.sumid
  const betsummarycode = await Betsummaries.find({sumid: req.body.sumid}).sort({date:-1}).exec()
  res.json({ betsummarycode: betsummarycode})
}))
router.put('/setbetstate', wrapAsync(async (req, res) => {
    let rbb = req.body.betdetail
    const result = await Bets.findOne({betid: rbb.betid}).exec()
...
router.put('/setbetsummarystate', wrapAsync(async (req, res) => {
  ...
  let ope = ''
  let des = ''
  const cbss = await Betsummaries.findOne({sumid:betsum.sumid},'sumid user admin state role adminpay').exec()
  const finduser = await Users.findOne({$or: [{user:betsum.admin}, {user:betsum.superadmin}]},'user admin autopay').exec()
  ...
  if (cbss.state === 'Aktif' && betstate === 'Kaybetti') {
    ...
    if (finduser.autopay == 1) {
      const cbs = await Bets.findOne({sumid:betsum.sumid, state: 'iade'}).exec()
      if (cbs) {
        const calcDraw = await Bets.findOne({betid:betsum.sumid, state: 'Kazandi'}, 'rate').exec()
    ...
    const oldearn = await Betsummaries.findOne({sumid:betsum.sumid}, 'earn').exec()
  ...
  async function insertGameLog(user, admin, superadmin, earn, ope, des) {
    const users = await Users.findOne({user: betsum.user}, 'creditremain').exec()
    let gamelog = new Gamelogs({
      gameid: betsum.sumid,
      user: user,
      admin: admin,
...
router.put('/paymentbet', wrapAsync(async (req, res) => {
  ...
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'G1') {
    ...
    const cbss = await Betsummaries.findOne({sumid:betsum.sumid}, 'sumid state role adminpay').exec()
    const cbs = await Betsummaries.findOne({sumid:betsum.sumid, state:'iade'}).exec()
    if (cbs) {
      const calcDraw = await Betsummaries.findOne({sumid:betsum.sumid, state:'Kazandi'}, 'rate').exec()
      let newrate = calcDraw.reduce(function (acc, item) {
        return acc * item.rate
      }, 1)
      drawwon = (newrate * betsum.amount).toFixed(2)
      setTimeout(async () => {
        await Betsummaries.updateOne({sumid:betsum.sumid},{$set:{adminpay: 'yes'}})
...
router.post('/detailscomission', wrapAsync(async (req, res) => {
  if (req.body.userbox === 'Üyeler') {
    const betsummary = await Betsummaries.find({$or: [{user: req.session.auth.user, state: 'Kazandi'}, {admin: req.session.auth.user, state: 'Kazandi'}]}).sort({_id: -1}).exec()
    res.json({ betsummary: betsummary })
  }else{
    const betsummary = await Betsummaries.find({user: req.body.userbox, state: 'Kazandi'}).sort({_id: -1}).exec()
    res.json({ betsummary: betsummary })
  }
}))
router.post('/detailscomissionquickdatefilter', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1') {
    const datefilter = await Betsummaries.find({user: req.session.auth.user, state: 'Kazandi', date: req.body.quickdate}).sort({_id: -1}).exec()
    res.json({ code: 200, datefilter: datefilter })
  } else {
    if (req.body.userbox === 'Üyeler') {
      const datefilter = await Betsummaries.find({user: req.session.auth.user, state: 'Kazandi', date: req.body.quickdate}).sort({_id: -1}).exec()
      ...
      const datefilter = await Betsummaries.find({user: req.body.userbox, state: 'Kazandi', date: req.body.quickdate}).sort({_id: -1}).exec()
      res.json({ code: 200, datefilter: datefilter })
    }
  }
}))
router.post('/detailscomissiondatefilter', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1') {
    const datefilter = await Betsummaries.find({user: req.session.auth.user, state: 'Kazandi', date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
    res.json({ code: 200, datefilter: datefilter })
  } else {
    if (req.body.userbox === 'Üyeler') {
      const datefilter = await Betsummaries.find({user: req.session.auth.user, state: 'Kazandi', date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
      ...
      const datefilter = await Betsummaries.find({user: req.body.userbox, state: 'Kazandi', date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
      res.json({ code: 200, datefilter: datefilter })
    }
  }
}))
...
router.post('/totalinfo', wrapAsync(async (req, res) => {
  let seuser = req.session.auth.user
  let cluser = req.body.userbox
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
...
router.post('/gamelogs', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    const gamelog = await Gamelogs.find({user: req.session.auth.user}).sort({_id: -1}).limit(50).exec()
    res.json({ gamelog: gamelog })
  } else {
    const gamelog = await Gamelogs.find({user: req.body.userbox}).sort({_id: -1}).limit(50).exec()
    res.json({ gamelog: gamelog })
  }
}))
router.post('/gamelog/datefilter', wrapAsync(async (req, res) => {
  if (req.session.auth.role === 'U1' || req.session.auth.role === 'C1') {
    const datefilter = await Gamelogs.find({user: req.session.auth.user, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
    res.json({ datefilter: datefilter })
  } else{
    const datefilter = await Gamelogs.find({user: req.body.userbox, date:{$gte: req.body.datestart, $lte: req.body.datefinish}}).sort({_id: -1}).exec()
    res.json({ datefilter: datefilter })
  }
}))
