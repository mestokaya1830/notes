            ...
            <div class="basket-list" v-for="(sublist, index) in filterBy(list, searchBox)" :key="index">
              ...
              <div class="basket-right-list">
                ...
                <div class="ottli-bas ottli-bas-list" @click="addSlip(sublist.han2, 'Deplasman'+' '+sublist.han2attr, sublist, 'Maç handkap')">
                  ...
                  <span class="ottli-bas" @click="getOdds($event,sublist.fixtureid)"><i class="fas fa-plus-square sso cp cl-f"/></span>
              </div>
            </div>
          </details>
        </div>
...
export default {
  ...
  mounted (){
    if(localStorage.getItem('set') !== null){
      let br = JSON.parse(localStorage.getItem('set')).find(item => item.filterid === 3)
      if(!br){
        this.getLeagues()
