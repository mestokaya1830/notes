  ...
  <div>
     <header class="chat-header">
      ...
      <div class="chat-user-title cp">
        <span class="chat-header-text" @click="setUserList()">{{chatName | capitalize}}</span>
        <i class="fas fa-window-close chat-close" @click="closeChat()"/>
      </div>
    </header>
    <div class="chat-user-side">
      <ul v-if="userList" class="chat-user-scroll">
        <li v-for="(list, index) in chatUsers" :key="index" class="chat-user-list" @click="updateMsg(list.user, list)">
          ...
          <span class="badge bg-g chat-count" v-if="list.count && list.count.length > 0">{{ list.count[0].count }}</span>
        </li>
      </ul>
