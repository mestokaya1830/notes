      ...
      <form class="reg-form">
        ...
        <div class="resetaccount-items">
          ...
          <span>
            ...
            <span class="subcredit cl-r" v-if="$store.state.auth.role === 'A1' && role && role.role === 'G1'">({{subCreditInfo.subcredit | currency('', 0)}})</span>
          </span>
        </div>
        <section>
          ...
          <footer class="form-footer" v-if="checkA1 || role.role === 'C1'">
...
export default {
  ...
  methods: {
    ...
    async getResetInfo() {
      try {
        if (this.$store.state.auth.role === 'A1' && this.userBox === 'Üyeler' || this.$store.state.auth.role === 'G1' && this.userBox === 'Üyeler') {
