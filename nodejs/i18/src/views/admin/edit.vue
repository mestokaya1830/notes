    ...
    <div class="container">
      ...
      <form @submit.prevent id="form" class="border rounded p-4">
        <div class="mb-3 row">
          ...
          <img :src="`/uploads/${post.imageName}`" alt="" class="updateImage">
          <div class="mt-3">
...
export default {
  ...
  methods: {
    ...
    async updatePost() {
      ...
      if(this.file !== null){
        this.data = new FormData()
        this.post.imageName = `${this.$store.state.auth._id}.${this.file.name}`
      ...
      if(this.post.title == ''){
        ...
        await axios.post('/api/admin/update', this.data).then(res => {
          if (res.status == 200) {
            this.error = {}
